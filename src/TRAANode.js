...
import { float, If, Fn, max, texture, uniform, uv, vec2, vec4, convertToTexture, passTexture, velocity, getViewPosition, viewZToPerspectiveDepth, struct, ivec2, mix, logarithmicDepthToViewZ, viewZToOrthographicDepth } from 'three/tsl';
...
class TRAANode extends TempNode {
	...
	setup( builder ) {
		...
		// Lottes max3: mix in compressed RGB, restore with the exact inverse.
		const flickerReduction = Fn( ( [ currentColor, historyColor, currentWeight ] ) => {
			...
			const blended = mix( compressedHistory, compressedCurrent, currentWeight );

			return blended.mul( float( 1 ).div( float( 1 ).sub( max( blended.r, blended.g, blended.b ) ) ) );
